...
export default async function handler(request, response) {
    ...
    if (["mercury", "venus", "earth", "mars"].includes(planetNameLower)) {
        prompt = `A highly detailed, photo-realistic landscape view from the surface of the planet ${planet}. Based on scientific data (${planet === 'Mars' ? 'reddish rocks and soil, thin hazy atmosphere' :
            planet === 'Venus' ? 'scorching hot surface with thick yellowish clouds, intense atmospheric pressure' :
                planet === 'Mercury' ? 'cratered surface similar to the Moon, extreme temperature variations, no atmosphere' :
                    'varied rocky terrain'}). Daytime, conditions appropriate for the planet. Vast perspective, wide angle view.`;
    }
        ...
        const result = await fal.subscribe(modelId, {
            ...
            onQueueUpdate: (update) => {
                ...
                if (update.status === "IN_PROGRESS" && update.logs && update.logs.length > 0) {
                    update.logs.forEach((log) => console.log(`${logPrefix} [Log]:`, log.message));
                }
                else {
...
//# sourceMappingURL=generateLandscape.js.map